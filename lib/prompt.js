  ...
  "slides": [
    {
      ...
      // items・listTitle/statement・subtext/stages・summary/lanes・stageLabels等）を実質的な内容として追加すること。使わないキーは省略可。
    }
  ]
}
...
- "journey_map": 全体の道のり（パンくずナビ）と、その中の詳細工程を1枚で示したい場合。stages:[{label,current?}]（全体ステージ、3〜6個、現在地はcurrent:true）、steps:[{number,label,color?}]（詳細フロー、2〜5個、矢印で接続）。少なくともどちらかを2項目以上指定。summary任意（下部の結論1文）。
- "swimlane_flow": 複数の役割・担当者（顧客/担当者/システム等）にまたがる業務の流れを、誰がいつ何をするか可視化したい場合。stageLabels:[...]（工程名、2〜6個）、lanes:[{role,activities}]（役割ごとの行、2〜5個）。activitiesはstageLabelsと同じ長さの配列で、その役割がその工程で行うことを文字列、行わない工程はnull。
- "grid_layout": 上記の各bodyType（two_column/two_row/grid_layout自体を除く）を「部品」として自由な格子状に組み合わせたい場合（例:3カラムで表＋グラフ＋数値を並べる、大きなグラフの隣に小さな要素を2つ積む等）。gridCols（1〜4）、gridRows（1〜3）でグリッドの大きさを決め、cells:[{col,row,colSpan,rowSpan,heading?,part,...partに応じたデータ}]（2〜8個）で各セルの位置・大きさ・中身を指定する。colSpan/rowSpanで隣接セルを結合できる（既定は両方1）。partには上記いずれかのbodyType名を指定し、そのbodyType用のキー（bullets/table/stat/slices/items等）をそのセルオブジェクトに直接持たせる。headingを付けると青帯の小見出しが入る（任意）。two_column/two_rowでは表現しきれない3カラム以上やL字型の組み合わせに積極的に使うこと。
